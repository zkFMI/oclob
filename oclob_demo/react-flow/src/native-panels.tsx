...
export function EvidencePanel({ book }: { book: PublicBook }) {
  return <section className="native-panel" aria-labelledby="evidence-title">
    ...
    <dl className="native-evidence">
      <div><dt>公開順序</dt><dd>{formatInteger(book.sequence)}</dd></div>
      <div><dt>署名した計算ノード</dt><dd>{book.nodes.length} / 7</dd></div>
      <div><dt>有効期限</dt><dd>{displayTime(book.nodes[0].valid_until)}</dd></div>
      <div><dt>この更新の決済</dt><dd>{book.finality.length ? '決済後の記録を7ノードが確認' : '決済を伴わない板の更新'}</dd></div>
      {book.finality.length > 0 && <div><dt>DeFMI台帳の高さ</dt><dd>{formatInteger(book.finality[0].height)}</dd></div>}
    </dl>
    <details><summary>識別値と各ノードの署名鍵を確認</summary>
      <p>処理の識別値</p><code>{book.round}</code>
      {book.finality.length > 0 && <><p>決済記録の識別値</p><code>{book.finality[0].receipt}</code></>}
      <ul className="native-keys">{book.nodes.map(node => <li key={node.party}>MPC {node.party + 1}<code>{node.signer}</code></li>)}</ul>
    </details>
    <p className="native-muted native-small">APIサーバーが登録済みの鍵で署名・決済記録・有効期限を検査しています。ブラウザー自身による暗号検証や、各ノードの現在の生存確認ではありません。</p>
  </section>;
}
