...
二つの連続する取引は、無制限に同時注文を処理できることの証明ではない。[取消・期限切れと資産回収](NATIVE_LIFECYCLE_JA.md)は別の実ネットワーク試験で追加確認した。競合して古くなった更新前状態からの再開、長時間の常駐ワーカー、鍵交代、保存領域全体の巻戻し検出は引き続き必要である。
復元結果の `notes` は過去に取り込んだ受取権の一覧であり、未使用残高の一覧ではない。`own_asset_refund` は現在、法人内で復号した金額、ロック、DeFMIの使用済み印を照合し、正の残高がある未使用の自社資産の返金だけを選ぶ。ただし、その後の別の支出まで防ぐ資金予約ではないため、新規予約時にも同じノートの使用済み・金額不足を必ず再検証する。
