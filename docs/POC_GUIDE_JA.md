...
現在の配布物は研究用MVPです。実資産を扱う本番システムではありません。統合受入経路では、法人側が注文を7分割し、注文ごとの決済暗号鍵も3-of-7で分け、7つのMPCコンテナへ別々に暗号化して直接送ります。ノードは正しいMPC結果を保存した後だけ鍵片を返し、その結果を5検証者のAvalanche DeFMI決済まで接続します。ただし、受入済み構成は一台のLinuxホスト上であり、7つの独立運営者を証明するものではありません。照合後は一つの研究用決済プロセスが注文別鍵と注文全文をメモリへ復元します。ブラウザ用の単体デモも、調整サービスが注文を一時的にメモリへ持つ従来経路です。
