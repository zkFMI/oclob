...
現在の配布物は研究用MVPです。実資産を扱う本番システムではありません。統合受入経路では、法人側が注文を7分割し、7つのMPCコンテナへ別々に暗号化して直接送り、その結果を5検証者のAvalanche DeFMI決済まで接続します。ただし、受入済み構成は一台のLinuxホスト上であり、7つの独立運営者を証明するものではありません。研究用の決済ゲートウェイは注文者が事前署名した決済権限を単独で復号します。ブラウザ用の単体デモも、調整サービスが注文を一時的にメモリへ持つ従来経路です。
...
| zkPI範囲証明 | 7 party中3 contribution | 金額と価格が承認範囲内であることを共同証明する |
...
1. MakerとTakerが別プロセスで注文分割と固定長の決済権限を作る。
2. 7 MPCノードが保存受領証へ署名し、二つの照合ラウンドを実行する。
3. 決済ゲートウェイがDeKYX提示と、MPCへ分割した6項目との一致を検証する。
4. Makerの最大在庫予約をDeFMIで確定してからTakerの照合へ進む。
5. Taker予約と40口・価格100のDvPを一つの正本遷移で確定する。
6. 同一遷移の再送拒否、5検証者のroot一致、1検証者再起動後の復旧を確認する。

結果は `artifacts/oclob_distributed_avalanche_acceptance.json` に保存されます。これは中央調整役に平文注文を渡さない一続きの機能証拠です。ただし、`non_claims` にあるとおり、1ホスト、単一決済ゲートウェイ、研究用鍵の構成です。
...
- 分散MPCから実Avalanche L1までの1ホスト統合経路は受入済みだが、単一決済ゲートウェイ、独立validator、WAN、再編を解消していない。
