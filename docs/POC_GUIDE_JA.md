...
現在の配布物は研究用MVPです。実資産を扱う本番システムではありません。分散受入経路では、法人側が注文を7分割し、7つのMPCコンテナへ別々に暗号化して直接送ります。ただし、受入済み構成は一台のLinuxホスト上であり、7つの独立運営者を証明するものではありません。ブラウザ用の単体デモは、調整サービスが注文を一時的にメモリへ持つ従来経路です。
