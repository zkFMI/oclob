...
現在の配布物は研究用MVPです。実資産を扱う本番システムではありません。統合受入経路では、法人側が注文を7分割して7つのMPCコンテナへ別々に暗号化して直接送ります。約定後は各ノードの秘密共有だけから共同zkPI/DvP証明を作り、価格・数量・予約額を証明担当へ復元せず、5検証者のAvalanche DeFMI決済まで接続します。ただし、受入済み構成は一台のLinuxホスト上であり、7つの独立運営者を証明するものではありません。また正本更新の互換処理は注文別鍵と注文全文を照合後に開くため、共同証明だけを正本更新の唯一の根拠にする切替が残っています。ブラウザ用の単体デモも、調整サービスが注文を一時的にメモリへ持つ従来経路です。
