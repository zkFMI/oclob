...
証拠は `artifacts/oclob_native_notes.json`、再現方法は[実行手順](NATIVE_PRETRADE_DEMO.md)です。これは単一ホスト・1取引の機能確認です。法人キューとウォレットの復旧、複数約定・取消・期限切れの全経路、受領した権利の再利用、MPC側の確定確認、独立運営と本番APIは未受入です。従来の口座差分方式とブラウザ用の中央調整経路も残っており、新経路の保証をそのまま適用しません。
...
- 法人側で生成する新しい事前予約証明を、耐久キュー・再送・ウォレット更新と接続する。新経路の決済権限に注文原文は含めない。
...
- 法人キュー、複数約定、取消、期限切れ、同時注文を新しい予約・決済経路へ接続して確認する。
