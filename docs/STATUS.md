...
前段のDeFMI `33d0c97` を使った試験では、支出証明と一括決済口で、決済後の受取権3件の取込み、両法人の秘密残高の復元、返金された同じ保有記録を次の注文の資金へ使う従来経路を再確認しました。次の注文は7ノードでの受付までで、約定・決済はこの試験に含みません。再現方法は[ウォレットの実行手順](CORPORATE_WALLET_JA.md)です。現在は、取消専用の署名認可と法人キューの解放確認を含むDeFMI `5dd1f68` に固定しています。

さらに各MPCノードが、自分の照合記録と決済指図全文を、設定済みDeFMIの確定記録に照合してから板状態を進めるようにしました。7ノードすべての未確認・すり替え通知の拒否と、正常な決済から返金の再利用までを実試験しました。証拠は `artifacts/oclob_native_finality.json`、接続方法と信頼の範囲は[決済確認の説明](NATIVE_FINALITY_JA.md)です。常駐市場サービスを加えた現行ソースは、Softbank上で116件のRustテスト、整形、警告を許さない静的検査を通しています。
...
さらに[法人側のキューと常駐送信](CORPORATE_WORKER_JA.md)を接続しました。実ノードを一つ停止して送信待ちを確認し、予約確定後と7ノード受付後に法人プロセスを強制終了しました。再起動した常駐処理が保存済みの同じ指図で受付を完了し、その注文で上記の2回の決済と2回の資産解放を実行しました。二重ワーカーの拒否、同じ要求の再登録と常駐処理の再起動、7 MPCノードと5検証ノードの状態一致も確認しました。常駐市場を加えた最終ソースとDeFMI `5dd1f68` で、この全経路を再実行済みです。証拠は `artifacts/oclob_native_worker.json`、実行した73ファイルのハッシュは `artifacts/oclob_native_worker_sources.json` です。キューは7ノードの受付までを管理し、決済済みとは表示しません。
...
[資金証明を後から作る注文受付](DEFERRED_FUNDING_JA.md)を追加しました。法人が先に売買条件と資産確保を署名し、実際の準備はキューの順番が来てから最新の保有資産・保証枠を使って行います。接続設定を事前保存していれば、DeFMI停止中も社内の依頼を保存できます。同じ法人記録を共有する複数プロセスの受付を直列化し、枠不足の注文は資産確保前に終了します。元の承認、参加資格、署名をワーカーが作り替えることはありません。109件のRustテスト、整形、全対象の警告を許さない静的検査をSoftbankで確認しています。

[常駐市場サービス](NATIVE_MARKET_JA.md)では、法人キューから認証付き通信で3注文を受け取り、手動の売買相手指定なしに3回の順序確定・MPC処理と2件の一括決済を実行しました。高い売り注文が先でも安い注文を優先し、合計9,030円、14件のノード別確定確認となりました。実決済の直後に終了コード75で停止し、保存した同じ指図から同じ決済記録へ復旧。さらに処理完了後にも別プロセスへ入れ替え、二重決済がないことを確認しました。最終ソースでの証拠は `artifacts/oclob_native_market.json`、73ファイルの内容との対応は `artifacts/oclob_native_market_sources.json` です。

いずれも単一ホスト・少数注文での機能確認です。取消・期限切れを含む市場側の長期無人運転、証拠不足の予約の無人復旧、部分配送後の清掃、独立した法人記録による同時枠更新、独立運営と本番APIは未受入です。受付保存の途中でキューへ入る前に停止した要求は、同じ依頼番号で受付を再実行する必要があります。各ノードは決済を自分で確認しますが、照合先のDeFMI接続サービスへの信頼は残ります。新しい支出証明の独立した暗号監査と、旧保存状態からの移行審査も残ります。従来の口座差分方式とブラウザ用の中央調整経路も残っており、新経路の保証をそのまま適用しません。新しい市場への通信元の匿名化、価格帯別の公開板をMPCから作って画面へ配信する接続も未確認です。
...
| 価格・時間優先 | 実装済み・1ホスト機能確認済み | 売買両方向、同値の先着順、対象外注文など10入力例を実7者MPCと基準計算で比較。価格逆順で受け付けた注文を実決済 | WAN並行投入、公平な到着順の形式化 |
...
| 法人側の常駐送信 | 新経路で1ホスト機能確認済み | 保存済み予約と暗号文の再利用、実ノード停止、2か所の強制終了からの自動再開、OSロックによる二重起動拒否、受付後の再起動、同じ注文の実決済 | 独立した法人記録による同時枠更新、長時間運転 |
| 市場側の常駐処理 | 新経路で1ホスト機能確認済み | 認証付きの受領証通信、保存してから応答、受付順の自動処理、3ラウンドと2実決済、決済直後の停止から同一指図で再開 | 取消・期限切れの常駐化、公開板・API・画面、証明途中や期限切れ計画の復旧、通信元の匿名化 |
| 停止中の受付と共有保証枠 | 新経路で1ホスト機能確認済み | DeFMI一時停止・全7 MPC停止中の3件保存、2プロセス同時投入、枠不足1件の確保前終了、元の承認を使う2件の受付と一括決済 | 受付途中の無人回収、独立した記録の統合、送信不明の資金証明の扱い、複数の保有記録を集める資金選択 |
| 未受付注文の期限切れ回収 | 新経路で1ホスト機能確認済み | 全7ノード停止、未送信終了と送信開始の競合排除、確保応答喪失、実解放直後の終了から復旧、元資産の再利用、5検証ノードの再起動 | 応答不明かつ正本の確保なし、旧追跡なし記録、同時更新、外部解放の受領証発見、部分配送の清掃 |
...
- 法人CLIと常駐送信の保存・再送、複数約定後の資産回復、次の約定、残り注文の取消・期限切れ、未受付予約の条件付き自動解放まで接続済み。次は証拠不足の要求の照合、部分配送の清掃、市場側の常駐処理と同時更新時の無人復旧をつなぐ。新経路の決済権限に注文原文は含めない。
