...
## 2026-09-06：注文クライアントの保存領域を分離

注文クライアントは再送用の暗号化記録だけを持ち、法人APIの処理キューや保存記録を直接参照しない構成にしました。保存用の鍵も別々です。この構成で4注文、2約定・合計7,500の決済、残る拘束量45／0、他社アクセス拒否、APIと検証ノードの再起動を確認しました。132件のRustテストと静的検査は成功し、実行した171ファイルとの対応を `artifacts/oclob_native_corporate_client_sources.json` に保存しています。クライアントには引き続き自社の署名・資産設定があり、ブラウザー認証やハードウェア鍵への移行が完了したわけではありません。

## 2026-09-06：法人ごとの認証付きAPI

[法人API](CORPORATE_API.md)を別コンテナとして追加しました。登録済みの法人クライアントだけが、署名済み注文のキュー登録、処理状況、自社の台帳上の資産・保証枠を確認できます。公開板APIや市場側に法人の鍵を渡しません。実際の4注文をAPI経由で処理し、2約定・合計7,500の決済、他社キューへのアクセス拒否、API再起動後の再送を確認しました。拘束残高は当初の額面120ではなく、現在の予約残量45であることを実APIで確認しています。131件のRustテストと静的検査は成功しました。最終ソース170ファイルの一致と結果ファイルの一致は `artifacts/oclob_native_corporate_api_sources.json` に記録済みです。ブラウザーの法人認証・操作画面と、未取込み受取権を含む資産表示は引き続き未完了です。

## 2026-09-06：実ネットワークの公開板を画面へ接続

[読み取り専用のReact Flow画面](NATIVE_BROWSER.md)を追加しました。法人の秘密情報を持たないAPIが実際のMPC公開板を検証し、価格・残数量・7ノードの署名・決済後の記録を表示します。Softbankで129件のRustテスト、5件の画面単体テストと全関連型チェック・ビルドが成功しました。実ネットワークの2件の決済結果をPCと390px幅のブラウザーで確認し、配信終了時は古い価格を消すことを確認しました。[画面の確認記録](NATIVE_BROWSER_AUDIT_20260906.md)を参照してください。以下の過去の「画面未確認」は、この限定された読み取り専用経路について更新されます。法人認証・注文・残高操作を含む全体の画面受入は引き続き未完了です。
...
さらに各MPCノードが、自分の照合記録と決済指図全文を、設定済みDeFMIの確定記録に照合してから板状態を進めるようにしました。7ノードすべての未確認・すり替え通知の拒否と、正常な決済から返金の再利用までを実試験しました。証拠は `artifacts/oclob_native_finality.json`、接続方法と信頼の範囲は[決済確認の説明](NATIVE_FINALITY_JA.md)です。公開板のHTTP接続を加えた現行ソースは、Softbank上で127件のRustテスト、整形、警告を許さない静的検査を通しています。
...
`artifacts/oclob_native_notes.json` と `artifacts/oclob_native_recovery.json` は、以前の依存版での接続・停止復旧の記録です。旧支出証明には[安全性上の欠陥](https://github.com/zkFMI/defmi/blob/3aadc4577129ce74347082347915484764157715/docs/NOTE_PROOF_SECURITY_REVIEW_20260905.md)が見つかっており、旧正常系の成功を修正版の安全性の証拠として流用しません。
...
[MPCからの公開板配信](PUBLIC_DEPTH_JA.md)では、同じ価格の残数量を秘密計算内で合算し、元の注文位置を隠して整列します。法人の鍵や市場の暗号化記録を持たない閲覧コンテナから4回取得し、100円の売り合計90が、合計75の2約定後に15へ減ることを確認しました。決済直後の実停止では古い板を保持し、同じ指図で復旧して全ノードの確定確認後だけ新しい板へ更新。5検証ノードの再起動後も一致しました。最終ソースの結果は `artifacts/oclob_native_depth.json`、実行した83ファイルとの対応は `artifacts/oclob_native_depth_sources.json` です。これは読み取り専用TLS接続であり、HTTP APIやReact Flowの接続完了ではありません。

[読み取り専用HTTP API](PUBLIC_BOOK_HTTP.md)は、法人の秘密鍵や市場の保存記録を持たず、既存のTLS経路から署名付き公開板を取得して検証します。最終ソースで4段階の板の完全一致、未作成・不正経路・POST・不正引数・未確定の連番・配信元の実停止に対する6応答、実決済後の停止復旧と5検証ノードの再起動を確認しました。127件のRustテストと静的検査も合格。結果は `artifacts/oclob_native_http.json`、87ソースファイルの対応は `artifacts/oclob_native_http_sources.json` です。注文・取消の取引APIやReact Flow接続の完了ではありません。

いずれも単一ホスト・少数注文での機能確認です。取消・期限切れを含む市場側の長期無人運転、証拠不足の予約の無人復旧、部分配送後の清掃、独立した法人記録による同時枠更新、独立運営と本番APIは未受入です。受付保存の途中でキューへ入る前に停止した要求は、同じ依頼番号で受付を再実行する必要があります。各ノードは決済を自分で確認しますが、照合先のDeFMI接続サービスへの信頼は残ります。新しい支出証明の独立した暗号監査と、旧保存状態からの移行審査も残ります。従来の口座差分方式とブラウザ用の中央調整経路も残っており、新経路の保証をそのまま適用しません。新しい市場への通信元の匿名化、公開板のHTTP APIから画面への接続も未確認です。MPCによる集計と読み取り専用TLS配信の検証は、画面の検証とは区別します。
...
| 市場側の常駐処理 | 新経路で1ホスト機能確認済み | 認証付きの受領証通信、保存してから応答、受付順の自動処理、3ラウンドと2実決済、決済直後の停止から同一指図で再開 | 取消・期限切れの常駐化、HTTP API・画面、証明途中や期限切れ計画の復旧、通信元の匿名化 |
...
| 公開板 | 新経路で1ホスト機能確認済み | MPC内の同値合算・整列、7署名と決済確定の結合、実TLSとHTTPで各4回の取得、決済直後の停止と復旧 | 画面・HTTPS接続口、注文がない時間の更新、負荷制御、差分漏洩測定 |
...
| 本番HTTP/API | 部品実装・未受入 | 新しいDeFMI接続口は相互TLS、固定した接続者と役割、許可したAPIだけの転送、通信量上限を実装。公開板のみ独立HTTP APIを実機検証。ブラウザは従来のデモAPI | 本番の法人登録、認可・監査、送信頻度制限、運用管理、画面との接続 |
