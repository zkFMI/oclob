...
OCLOBは、研究用MVPとして、法人側の注文分割、7 MPCノードの秘密照合、ノードごとの秘密板の持越し、秘密共有からの共同zkPI/DvP証明、Maker予約、Taker予約とDvP、5検証者の非EVM DeFMI Avalanche L1確定までを一続きに実装し、1ホスト上で受入済みです。約定前に全注文を開ける単一の決済鍵と、次回照合のために中央で平文板を持つ必要は廃止しました。共同証明の生成からDeFMIの残高commitment更新まで、価格・数量・予約額の中央復元は不要です。共同zkPI、一回限りの番号、DvP packageがMPC側と正本命令側で同一であることも受入時に検査します。ただし共同証明全文の検証は現在OCLOB決済アプリ内で行い、Avalanche VMはDeFMI承認委員会が署名した要約と口座差分を検証します。本番移行には、到着注文の事前予約証明から照合後の決済権限開示を除くこと、各承認者による独立証明検証またはVM内検証、独立したMPC/validator運営者、鍵・証明nonceを含む世代整合、本番APIの認証・運用制御がP0です。
...
| 共同zkPI/DvP証明 | 実装済み・1ホスト受入済み | 7ノードが各自の616秘密値だけから金額・価格範囲、数量×価格、3種の決済後残高、Maker予約残を証明。3-of-7署名、受取人別暗号化opening、署名済みMPC結果への固定、正本入力との同一性を検証 | 独立prover、全nonceの世代整合、到着注文の予約証明も共同生成 |
| DeFMI原子的DvP | 実装済み・統合1ホストL1受入済み | 共同証明の要約を正本命令へ直接固定しMaker/Taker予約と残高commitmentを更新。5 validator root一致、replay拒否、再起動復旧 | 各承認者またはVMでの証明全文再検証、独立validator、reorg試験、account-free note方式 |
...
R1〜R5として列挙しています。受付証明の分離、要約値の秘密乱数の変更、市場の固定、
予約固有の永続的な使用記録を実装していますが、実Avalanche上の匿名ノート予約から
決済までを結ぶ経路の受入はまだ残っています。
...
- 到着注文の事前予約zkPIを法人またはMPC側で作り、照合後の決済権限開示を不要にする。
...
現在、各約定について7ノードが自分の616個の秘密共有だけを読み、金額・価格範囲、数量×価格、証券・資金・Maker予約の残りを共同証明します。証明専用の相互TLS口は、ノード署名付きのラウンド、約定枠、公開MPC結果、ファイル要約値、そこから一意に決まる処理番号を検査します。統合受入では、価格・数量・予約額・暗号用乱数を決済担当へ復元せず、3-of-7共同署名付きzkPI、DvP証明、受取人別openingを生成・検証しました。

OCLOB決済アプリは共同証明の委員会公開鍵を起動時に固定し、注文時に受理した正確な予約commitmentを予約状態rootへ含めます。約定後は、共同証明内のzkPI、一回限りの番号、DvP packageを作り直さず検証し、資金・証券commitmentとMaker/Taker予約残を含む正本命令を作ります。統合受入では、MPC側と正本命令側の三つの要約値が完全に一致することを検査します。Avalanche VMが独立に検証するのは、その命令に対するDeFMI承認委員会の署名です。

注文ごとの決済暗号鍵を3-of-7で解放する経路は、到着注文の事前予約を認可するために残っています。これはDvPを作り直すためではありません。現状の事前予約zkPIは、開いた決済権限からDeFMI側が組み立てます。

必要な変更:

- 到着注文の事前予約zkPIを法人またはMPCから受け取り、注文別鍵の照合後開示を除く。
- 現在一プロセスにあるDeFMI承認鍵を独立サービスへ分け、各承認者が共同証明と正本命令を検証してから署名する。別案としてRust VM内へ検証器を入れる。
