...
証拠は `artifacts/oclob_native_notes.json`、再現方法は[実行手順](NATIVE_PRETRADE_DEMO.md)です。法人側の暗号化記録を使った停止後の再送も、`artifacts/oclob_native_recovery.json` の経路で確認しています。いずれも単一ホスト・1取引の機能確認です。常駐ワーカーと約定後のウォレット復旧、複数約定・取消・期限切れの全経路、受領した権利の再利用、MPC側の確定確認、独立運営と本番APIは未受入です。従来の口座差分方式とブラウザ用の中央調整経路も残っており、新経路の保証をそのまま適用しません。
...
| DeFMI原子的DvP | 新経路で1ホスト機能確認済み | 両法人の確定済み匿名ノート予約を消費。各VMが証明全文を検証し、5ノードの台帳一致、二重適用なし、1ノードの再起動復旧を確認 | 複数約定・取消・期限切れ・同時注文の全経路、受領権の再利用、独立検証者、再編試験 |
...
- 法人CLIの保存・再送は接続済み。常駐ワーカー、期限切れ解除、約定後のウォレット更新までつなぐ。新経路の決済権限に注文原文は含めない。
...
- 受領した権利を再利用可能な匿名保有記録へ変換し、悪意ある暗号化断片やノード停止にも対処する。
