...
出力先も別にします。標準ではMaker/Takerの固定例です。所有者だけが読める
`OCLOB_CORPORATE_ORDER_FILE` で別の指値・数量・資金ノートを指定する経路もあります。
ただし、認証済みの企業向け注文受付APIや常駐ワーカーが完成したという意味ではありません。
...
- 約定後の保証枠・受取権から次の注文の資金を復元する処理は[ウォレット経路](CORPORATE_WALLET_JA.md)を参照してください。取消・期限切れ・複数約定を組み合わせた回復は別の受入対象です。
