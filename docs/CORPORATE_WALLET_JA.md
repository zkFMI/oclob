...
- 支出証明はDeFMI `153fe67` の新形式を使う。固定した外部実装は実験用であり、独立した暗号監査の合格を意味しない。[DeFMIの安全性レビュー](https://github.com/zkFMI/defmi/blob/3aadc4577129ce74347082347915484764157715/docs/NOTE_PROOF_SECURITY_REVIEW_20260905.md)を必ず確認する。旧証明・旧保存状態を黙って受け入れる自動移行は提供しない。
