...
DeFMI `e28a2e7` では、役割を固定しない予約指示、DeKYX の検証、
...
[DeFMI の接続資料](https://github.com/shukob/defmi/blob/e28a2e7f5c54f2901bd20327fc297ef6d40a4ea8/docs/APPLICATION_NOTE_RESERVATIONS_JA.md)
に記載しています。
...
この署名処理と決済文の組み立てを追加しても、以下の Docker 送信・約定時消費・解除まで接続済みにはなりません。

1. 法人端末で匿名ノートの予約を実際に確定し、正本読戻しを使って証明を取得する。
2. Docker/CLIの注文送信をこの受付証明と暗号化予約情報へ切り替える。
3. 実際の調整役で共同 DvP 証明の完了を各ノードに保存させ、`prepare_native_fill` → `certify_native_fill` を呼ぶ。旧来の注文全文を開く処理は、この経路では使わない。
4. 証明全文を検証する DeFMI VM の `issueApplicationNoteFill` / `issueApplicationNoteRelease` を実際の決済経路から呼び出す。
5. 実ネットワークで再送、部分約定、取消、期限切れ、同時注文、再起動を確認する。
6. 最終受領権を支出可能ノートへ変換する処理も、受領者の匿名性を保ちながら証明全文を VM で検証する。既存の変換は承認者への信頼が残る。
