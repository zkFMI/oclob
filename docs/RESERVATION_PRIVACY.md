...
1. 法人CLIの予約・暗号文・受領証の保存と、停止後の再送は接続済みです。残るのは常駐ワーカー、期限切れの解除、約定後の秘密残高の更新です。[保存と再開の条件](CORPORATE_RECOVERY_JA.md)を参照してください。
2. 複数約定、`issueApplicationNoteRelease` による取消・期限切れ、同時注文を実経路で確認する。
3. MPC側の秘密板を進める際の正本受領証の独立検証と、停止・再起動時の世代整合を確認する。
4. 最終受領権を支出可能ノートへ変換する処理も、受領者の匿名性を保ちながら証明全文をVMで検証する。既存の変換は承認者への信頼が残る。
