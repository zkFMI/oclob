...
DeFMI `4da3925` では、役割を固定しない予約指示、DeKYX の検証、
...
[DeFMI の接続資料](https://github.com/shukob/defmi/blob/4da39258c2a28f1b551ea371afd9e34ad2b46040/docs/APPLICATION_NOTE_RESERVATIONS_JA.md)
に記載しています。
...
委員会の実行に依存します。新しい決済依頼への署名は、各ノードが実際に
実行した MPC 計算・予約条件に結び付ける必要があります。
任意のメッセージに署名する処理で代用してはいけません。
...
3. MPC の共同 DvP 証明と、実行結果だけを承認するノード署名を、新しい `ApplicationNoteFill` へ接続する。
