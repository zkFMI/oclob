...
公開値とは別に、各partyは8枠と到着注文の `(有効状態, 売買方向, 価格, 残量)`、計36個のShamir評価だけを自分の永続領域へ保存します。受領証が署名する親状態と出力ファイルの要約値が一致し、対応するDeFMI遷移が確定した場合だけ、次回の `secret_input()` は元注文ではなくこの更新後shareを読みます。ファイル本体、保存先、shareは調整役や決済コンテナへ渡しません。
...
| `oclob-node` | 自ノード宛て二つの暗号文、公開ラウンド計画、DeFMI確定要約 | 署名付き保存受領証、署名付き公開結果、秘密板head確定受領証、結果後の署名付き決済鍵片 | 自分の注文1分割、更新後の板1分割、鍵1片だけを必要時に開く |
| `oclob-ordering` | commitment | 5-of-7受付証明 | 各ノード署名鍵 |
| `oclob-mpc` | 秘密入力share | 公開約定結果とparty別の更新後秘密板share | 各ノードは一つのshareのみが本番形 |
