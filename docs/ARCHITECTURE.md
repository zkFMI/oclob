...
4. **決済権限**: 注文者が事前署名した注文全文とDeKYX提示を、固定長暗号文として決済先だけへ渡す。研究用の現構成では一つの決済ゲートウェイが開く。

分散受入経路では1と2を別プロセス・別コンテナへ分離し、法人側から各MPCノードへ一つずつ暗号化して直接送ります。各ノードは保存した注文要約値と状態世代を署名して返します。公開要約は注文ごとの使い捨て鍵で署名するため、調整役は注文全文だけでなく法人の長期署名鍵も受け取りません。6つの照合項目は、MPC用分割値と決済権限内の値が同じであることをPedersen VSSで検査します。ただし、現在の受入は一台のホスト内であり、決済権限は一つの研究用ゲートウェイが開きます。別運営者・別ホスト・WAN・閾値決済権限での受入が必要です。ブラウザ用の単体デモは一つのプロセス内で動く従来経路なので、この中央非開示保証の対象外です。
...
sequenceDiagram
    ...
    U->>M: 検証可能な7分割を各ノードへ直接暗号配送
    M-->>U: 7件の署名付き保存受領証
    U->>G: 事前署名済み決済権限を固定長暗号文で配送
    U->>O: 注文全文ではなくcommitment
    O-->>U: 受付番号と5署名
    M->>M: 受付順どおり価格・時間優先照合
    M-->>P: 約定、板差分、前後状態の要約値
    G->>G: DeKYXとMPC分割値との一致を検査
...
現受入では、決済ゲートウェイが注文者宛てではなく自分宛ての固定長暗号文を開き、参加者署名、DeKYX提示、注文commitment、MPC用VSSの6定数項を確認してからzkPIを組み立てます。このため調整役は注文を知りませんが、決済ゲートウェイは知ります。本番候補では、同じ公開インターフェースを保ったまま、MPCノードによる閾値復号または共同zkPI生成へ置き換える必要があります。
...
| `oclob-edge` | 法人側の秘密注文、7ノード公開鍵 | 公開要約値、ノード別の固定長暗号文 | 注文全文を分割完了まで持つ |
| `oclob-node` | 自ノード宛て暗号文、公開ラウンド計画 | 署名付き保存受領証、署名付き公開結果 | 自分の1分割だけを実行時に開く |
...
| `oclob-settlement` | 検証済み決済権限、約定、予約、遷移証拠 | zkPI、DeFMI受領証 | 研究用ゲートウェイでは決済権限と口座openingを保持 |
