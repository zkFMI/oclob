...
ワーカー自身は注文、本人確認資格、予約の署名を新しく作らない。法人があらかじめ保存した署名済み条件を扱う。[資金証明を後から作る方式](DEFERRED_FUNDING_JA.md)では、処理順が来たときだけ、元の署名を使って現在の資産・保証枠に対する証明を作る。一度準備・送信した指図や暗号文は保存済みのものを使う。取引所の調整役へ法人の鍵や注文原文を渡す仕組みではない。

この部品の完了は「7ノードが同じ暗号文を受け付けた」までである。実際の約定、共同zkPI、DeFMIでの決済は後続の市場処理で行う。キューの受付済み表示を決済済みとして扱わない。

[常駐市場サービス](NATIVE_MARKET_JA.md)へ接続する場合は `OCLOB_MARKET_CONFIG` を指定する。7ノードの受領証と暗号化された決済権限を市場へ送って、保存済みという応答を受け取る。市場が停止していれば元の受領証を保持して再試行する。これも決済完了の通知とは区別する。`maker-worker` と `taker-worker` の両方で同じ仕組みを使う。
...
- `outbox.intake.lock`：複数の社内プロセスが、注文記録とキューへ異なる順で書き込むことを防ぐ受付用ロック。同じ法人記録を共有する構成で使う。
...
flowchart TD
    ...
    B --> T{"注文の期限が過ぎている？"}
    T -- はい --> X["正本の予約を確認して解放\n未送信の証拠がある場合だけ送信せず終了"]
    T -- いいえ --> C{"7ノードの受付経路へ接続できる？"}
    C -- いいえ --> W["待機：指図と順序を保持"]
    W --> T
    C -- はい --> D["送信開始を記録し、同じ予約指図をDeFMIへ送る"]
    D --> E["正本台帳で予約確定を確認"]
    E --> F["保存した同じ7個の暗号文を配送"]
...
注文をキューへ登録するには、資金証明を先に準備する従来方式では `OCLOB_NATIVE_ENQUEUE=1`、最新の枠を後から使う方式では `OCLOB_NATIVE_ENQUEUE=authorized` を指定する。後者は最初に認証済みの接続設定を取得して保存する。[具体的な受付手順](DEFERRED_FUNDING_JA.md)を参照。`OCLOB_CORPORATE_REQUEST_ID` は社内システムで発行した安定した依頼番号とし、再実行でも同じ番号を使う。以下は従来方式の例である。
...
oclob-corporate-worker --wait-reconciled native-expiry-release-002
```
...
`OCLOB_NATIVE_ENQUEUE=authorized` では、接続設定を保存済みならDeFMI停止中でも署名済み条件を預かれる。ワーカーは処理順が来てから現在の資産と保証枠を読む。資格自体を後から発行する方式ではない。独立した複数の法人記録による同時更新、資金証明を既に送ったか不明な要求の再作成、受付保存の途中でキュー登録前に止まった依頼の完全な無人回収は残る。旧 `=1` 方式は引き続き受付時のDeFMI読み取り接続が必要である。

未受付のまま期限切れになった要求は、[法人側の期限切れ回収](QUEUED_EXPIRY_JA.md)で扱う。MPCが全停止中でもDeFMIでの確保と実解放を確認して、元の資産を復元できる。未送信のまま終了するには、送信開始と同時には成立しない永続記録が必要である。応答不明を未送信に置き換えない。証拠不足・同時枠更新・一部ノードだけへの配送の清掃は未解決として残す。受付済み注文の取消・期限切れは [市場側の経路](NATIVE_LIFECYCLE_JA.md)で検証している。
...
2026年9月6日（日本時間）の `oclob-native-worker-market-regression-006` は、常駐市場と秘密の価格優先計算を加えた最終ソースとDeFMI `5dd1f68` で、従来の準備済み要求を使う通し試験をSoftbankで完了した。先行する試験結果を流用したものではない。この回帰試験の取消・期限切れ処理は従来の調整プログラムで実行しており、常駐市場で取消まで自動化できたという証拠ではない。
...
- 別の回帰検査でRustテスト116件、整形、全対象の警告を許さない静的検査を完了。

[公開した実行結果](../artifacts/oclob_native_worker.json)と[73ファイルのソース対応表](../artifacts/oclob_native_worker_sources.json)を参照。注文原文、秘密鍵、暗号化キューの実ファイルは公開しない。1台のホスト上の機能確認であり、独立運営・WAN・性能比較・本番移行の合格ではない。
