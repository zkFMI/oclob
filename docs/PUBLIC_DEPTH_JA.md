...
検証状況：最終ソースによる実MPC・実DeFMIの通し試験がSoftbankで合格。4回の公開板の取得、2件・合計7,500の実決済、決済直後の停止と復旧、5台の検証ノードの再起動を確認した。Rustの127テスト、整形検査、全ターゲットのClippy検査も合格。既存の法人ワーカー・取消・期限切れ・資金回収の回帰試験も合格。2回の決済・計3約定、2回の解放、受取権9件、保証枠更新番号8/5と、実プロセス停止・MPCノード再起動・検証ノード再起動後の一致を確認した。画面への接続、本番運用品質は未確認である。
...
入力は1 KiB、応答は128 KiBの固定長JSONレコードで、TLS 1.3・CA検証・ホスト名検証・証明書の固定を使う。この接続自体はHTTPではない。[独立したHTTP接続層](PUBLIC_BOOK_HTTP.md)がこのTLS経路を読み直して検証し、公開板をJSONで返す。React Flow画面への接続は残る。取引受付側の相互TLSは変更しない。
...
常駐サービスへの取消・失効・資金解放の統合、公開板を用いた実画面、注文・取消などの取引API、注文のない時間帯の更新、複数サービスへの分散、過去回路の保存状態からの移行、公開接続の負荷制御、通信の匿名性、独立監査は未完了。既存の全体作業目標を、この機能の試験だけで完了扱いにしない。
