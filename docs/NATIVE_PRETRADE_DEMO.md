...
- 法人の`queue/outbox.enc`と`outbox-key.raw`：再送に使う暗号化記録と、その鍵。別々に保存し、法人側だけへ渡します。初期化と再開は別操作です。
...
法人側の予約・送信記録は永続化し、プロセス停止と一部ノードへの送信失敗からの再開も確認しています。[法人側の保存と再開](CORPORATE_RECOVERY_JA.md)を参照してください。常駐ワーカー、約定後のウォレット更新、複数約定・取消・期限切れの一貫した運用、悪意のある暗号化断片への対処、独立運用者での負荷・障害試験は残っています。単体試験やこの1シナリオの成功だけでは、製品化完了とはしません。
