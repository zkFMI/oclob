...
> **現在の到達点**
> 研究用MVPです。価格・時間優先、部分約定、複数約定、GTC、IOC、取消、期限切れ、法人単位の資金・在庫予約、DeKYX参加資格、閾値zkPI、原子的DvP、暗号化耐久キュー、React Flowデモまで実装しています。分散受入経路では、法人側の注文分割、各ノード専用の暗号配送、相互TLS、ノードごとの保存領域、1コンテナ1プロセスのMP-SPDZ実行、署名付き結果、再送時の再実行防止まで動作確認済みです。ただし、現在の受入は1台のLinuxホスト上の7コンテナであり、7社による独立運用ではありません。DeFMI決済も実Avalanche L1への接続前です。
