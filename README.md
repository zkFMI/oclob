...
- **MPC:** the official MP-SPDZ compiler and `malicious-shamir-party.x`. Each resident node opens only its own share for input. Falling back to plaintext matching is prohibited. Since 2026-09-08 the single-process runner (`oclob-mpc::MpcRunner`, used by the demo server) keeps its seven parties alive between rounds and feeds each round through named pipes, so the 42 mutually authenticated TLS handshakes are paid once per mesh rather than per order; the node executor of the seven-process cluster still spawns its party per round.
