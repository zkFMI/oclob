...
- 現在の統合受入では、一つの決済ゲートウェイが参加者署名済みの固定長暗号文を開き、MPCへ渡した6項目と一致することを検証します。したがって「調整役に見えない」は確認済みですが、「決済ゲートウェイにも見えない」は未達です。本番候補では決済権限の閾値復号・共同生成へ置き換えます。
...
結果は `artifacts/oclob_distributed_avalanche_acceptance.json` に保存されます。この成果物は一続きの機能確認ですが、1ホスト構成、単一の研究用決済ゲートウェイ、デモ鍵という制限を明記しています。
