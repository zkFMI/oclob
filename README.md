...
> **現在の到達点**
> 研究用MVPです。価格・時間優先、部分約定、複数約定、GTC、IOC、取消、期限切れ、法人単位の資金・在庫予約、DeKYX参加資格、閾値zkPI、原子的DvP、暗号化耐久キュー、React Flowデモまで実装しています。一続きの受入経路では、法人側で作った注文分割を相互TLSで7 MPCノードへ直接届け、各ノードの署名付き保存受領証を検証しました。注文ごとの使い捨て署名鍵を使うため、調整役が受け取る公開要約には法人の長期鍵を含めません。決済権限の暗号鍵も注文ごとに新しく作り、3-of-7でMPCノードへ分散します。各ノードは、その注文を扱ったMPC結果を永続化した後だけ鍵の一片を決済担当へ返します。二片では復元できず、三片以上の正しい署名とcommitmentを検証して初めて開けます。受付時は7ノードが投票し、5票以上を含む証明本体と前証明との連続性を各ノードが検証・永続化した後だけMP-SPDZを実行します。その一致結果からMaker予約とTakerのDvPを5検証者の非EVM DeFMI Avalanche L1へ確定しました。全検証者のroot一致、同一遷移の再送拒否、1検証者再起動後の復旧も同じ実行で確認しています。ただし、これは一台のLinuxホスト上の研究構成です。7社の独立運用、WAN、HSM、約定後も注文全文を一か所へ復元しない共同zkPI生成を受入れたものではありません。
...
- 決済: 3-of-7の閾値範囲証明を含むzkPI、到着注文の予約と複数約定を一つの原子的DeFMI DvPとして処理、再実行を拒否。
...
- 現在の統合受入では、決済担当が正しいMPC結果の後に3ノード以上から注文別の鍵片を集め、参加者署名済みの固定長暗号文を開き、MPCへ渡した6項目と一致することを検証します。これにより、約定前に全注文を開ける単一の復号鍵はなくなりました。ただし、約定または板への正式掲載が決まった後は、一つの研究用決済プロセスが注文全文をメモリへ復元します。注文全文を一か所にも復元しない共同zkPI生成は未実装です。
