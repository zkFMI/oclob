...
- 新しい匿名保有記録の経路は、実際の事前予約・秘密照合・共同証明・DeFMI決済まで接続済みです。法人CLIの暗号化記録を使い、予約直後や一部ノードへの送信後の停止から同じ要求で再開できます。ただし、常駐ワーカー、約定後のウォレット更新、複数約定・取消・期限切れを含む全経路の統合確認は残っています。情報の分離は[予約証明の情報分離](docs/RESERVATION_PRIVACY.md)を参照してください。
...
- 新経路のRust VMは共同証明全文を検証します。ただし、MPC側の秘密の板を進めるための決済受領確認、悪意のあるノードが送る暗号化断片の検証、受領した権利から再利用可能な保有記録を作る処理には、別途確認・強化が必要です。従来の口座差分方式も互換性試験として残しており、新経路の証明と混同しません。
