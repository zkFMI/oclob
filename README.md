...
> **現在の到達点**
> 研究用MVPです。価格・時間優先、部分約定、複数約定、GTC、IOC、取消、期限切れ、法人単位の資金・在庫予約、DeKYX参加資格、閾値zkPI、原子的DvP、暗号化耐久キュー、React Flowデモまで実装しています。分散受入経路では、法人側の注文分割、各ノード専用の暗号配送、相互TLS、ノードごとの保存領域、1コンテナ1プロセスのMP-SPDZ実行、署名付き結果、再送時の再実行防止まで動作確認済みです。別のAvalanche受入経路では、5検証者の非EVM DeFMI L1上で、Makerの閾値zkPI予約を板登録前に確定し、Taker予約とDvPを一括確定し、全検証者のroot一致・再送拒否・1検証者再起動後の復旧を確認しました。ただし両受入はまだ別経路で、各々1台のLinuxホスト上です。7社による独立運用や、分散MPCから実L1までの一続きの受入を示すものではありません。
...
- ブラウザ用の単体デモは互換性のため従来の中央調整経路を使い、調整サービスが注文を一時的にメモリへ持ちます。市場運営者からも注文を隠す保証は、今回受入済みの分散経路に限ります。両経路の統合は未完了です。
- Avalanche受入は1ホスト上の5検証者です。独立運営、WAN、再編、分散MPC経路との同一実行、HSM保管の承認鍵は未受入です。現在の正本は匿名commitment口座と予約状態rootを使い、口座を持たないnote方式への移行は残っています。
