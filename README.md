...
- 事前予約の新しい受付部品は、台帳参照を含まない署名付き受付証明と、注文別に閾値暗号化した予約情報を分けます。金額の要約値にも新しい秘密乱数を加え、台帳との直接照合を防ぎます。この経路では決済権限に注文全文を入れません。ただしDocker統合デモの切替と匿名ノート消費への接続は未完了です。詳細は[予約証明の情報分離](docs/RESERVATION_PRIVACY.md)を参照してください。
